...
import getFee from '../util/fee.js'; //模拟流控引擎数据
const FEE = new getFee();
export default {
  ...
  data() {
    return {
      ...
      focusItems: [], // 后期添加非form making自有属性，可聚焦组件节点集合
  ...
  mounted() {
    for(let i=0;i<this.data.list.length;i++){
      this.hideData(this.data.list[i],i)
      this.getFocus(this.data.list[i]);
    }
    this.focusItems = document.querySelectorAll(".canFocus input");
    this.handelFocus()  
  },
  methods: {
    // 动态数据处理函数
    dynamicData(targetdata){
      ...
      var pdf = FEE.flow.platform;
      var user = FEE.flow.user;
      if(targetdata.options.defaultValue!="" && (targetdata.options.defaultValue)[0]=="@"){
    ...
    hideData(activedata,activeIndex) {
      ...
      if (activedata.hidden && activedata.hidden !== "") {
        if (eval(activedata.hidden)) {
          ...
          // 隐藏节点时获取到被隐藏节点input，从聚焦节点列表中删除
          var removeParent = this.$refs[activedata.model][0].$el;
          removeParent.classList.add("remove");
          var remove = document.querySelector(".remove input");
          [...this.focusItems].splice(activeIndex,1)
          console.log(this.$refs[activedata.options.model])
        } else if (this.hideIndex && !eval(activedata.hidden)) {
          activedata.options.hidden=false;
          // 显示节点时获取到被显示节点input，加入聚焦节点列表中
          var addParent = this.$refs[activedata.model][0].$el;
          addParent.classList.add("add");
          var add = document.querySelector(".add input");
          [...this.focusItems].splice(activeIndex,0,activeIndex+1)
        }
      }
    },
    // 监听表单数据改变
    ...
    getFocus(originData){
        // 显示且非禁用元素
        if(originData.options.hidden==false && !originData.options.disabled ) {
          var parent = this.$refs[originData.model][0].$el;
          parent.classList.add("canFocus")  //后期添加是否可聚焦类名
        }
    },
    // 光标操作控制流程
    handelFocus() {
      if(this.focusIndex<this.focusItems.length){
